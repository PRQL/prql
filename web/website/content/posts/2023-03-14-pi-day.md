...
SQL) and allows us to include raw SQL inside our PRQL queries. The reason this
is needed in the query above is because the semantics of PRQL are that once a
value has been updated, the new value becomes immediately available to the
following steps in the pipeline. Therefore when we set `a=b` in the `select`,
from that point onwards `a` has the same value as `b`. For the Fibonnaci
calculation we need the value of `a` from the previous loop iteration though.
SQL does not have those semantics so by "looking through" the PRQL to the
underlying SQL with the s-string, we can still access the value of `a` from the
previous iteration.
...
loop (
    ...
    derive [
      ...
      k_new = (k+1) % loop_len,
      q_new = loop_steps 0 ((10*s"{a}[{k}]"+q*(len-k+1))/(2*(len-k)+1)) (q/10) q q q,
...
Actually I cheated a tiny bit - if you leave out the final `select` then the
query compiles to SQL which works in DuckDB as it is. However adding the final
`select` statement leads the compiler to add some expression substitutions which
aren't resolved correctly. Therefore I compiled the query without the final
`select` and manually replaced the final `SELECT` clause in the SQL with the
following:
