...
loop (
    ...
    derive [
      ...
      k_new = (k+1) % loop_len,
      q_step_i = (10*s"{a}[{k}]"+q*(len-k+1))/(2*(len-k)+1),
      q_new = loop_steps 0 q_step_i (q/10) q q q,

      a_step_i = s"[CASE WHEN i=={k} THEN (10*{a}[i]+{q}*({len}-i+1))%(2*({len}-i)+1) ELSE {a}[i] END for i in generate_series(1,{len})]",
      a_step_1 = s"[CASE WHEN i=={len} THEN {q}%10 ELSE {a}[i] END for i in generate_series(1,{len})]",
...
Something you might have noticed is that there are some expressions surrounded by `s""`. This
is called an [s-string](https://prql-lang.org/book/language-features/s-strings.html) (s for
SQL) and allows us to include raw SQL inside our PRQL queries. We use this to include features from
DuckDB that haven't made it into PRQL yet, such as the Pythonesque [list comprehensions](https://duckdb.org/docs/sql/functions/nested.html#list-comprehension).
...
table_6 AS (
  WITH RECURSIVE loop AS (
    SELECT
      ...
      0 AS _expr_1,
      0 AS _expr_2,
      [2 for i in range(len)] AS _expr_3,
      0 AS _expr_4,
      0 AS _expr_5,
      '' AS _expr_6
    FROM
    ...
    SELECT
      ...
      CASE
        WHEN _expr_1 = 0 THEN ''
        WHEN 1 <= _expr_1
        AND _expr_1 <= len THEN ''
        WHEN _expr_1 = len + 1 THEN ''
        WHEN _expr_1 = len + 2 THEN _expr_13
        WHEN _expr_1 = len + 3 THEN _expr_14
        ELSE ''
      END
    FROM
      (
        SELECT
          ...
          CASE
            ...
            WHEN 1 <= _expr_1
            AND _expr_1 <= len THEN _expr_5
            WHEN _expr_1 = len + 1 THEN _expr_5
            WHEN _expr_1 = len + 2 THEN CASE
              ...
              ELSE _expr_2
            END
            WHEN _expr_1 = len + 3 THEN _expr_5
            ELSE _expr_5
          END AS _expr_7,
          CASE
            WHEN _expr_1 = 0 THEN _expr_4
            WHEN 1 <= _expr_1
            AND _expr_1 <= len THEN _expr_4
            WHEN _expr_1 = len + 1 THEN _expr_4
            WHEN _expr_1 = len + 2 THEN CASE
          ...
          CASE
            ...
            WHEN 1 <= _expr_1
            AND _expr_1 <= len THEN [CASE WHEN i==_expr_1 THEN (10*_expr_3[i] + _expr_2 *(len - i + 1)
          ) %(2 *(len - i) + 1)
          ELSE _expr_3 [i]
      END for i in generate_series(1, len) ]
      WHEN _expr_1 = len + 1 THEN [CASE WHEN i==len THEN _expr_2%10 ELSE _expr_3[i]
  END for i in generate_series(1, len) ]
  WHEN _expr_1 = len + 2 THEN _expr_3
  WHEN _expr_1 = len + 3 THEN _expr_3
  ELSE _expr_3
END AS _expr_9,
CASE
  ...
  WHEN 1 <= _expr_1
  AND _expr_1 <= len THEN (
    10 * _expr_3 [_expr_1] + _expr_2 * (len - _expr_1 + 1)
  ) / (2 * (len - _expr_1) + 1)
  WHEN _expr_1 = len + 1 THEN _expr_2 / 10
  WHEN _expr_1 = len + 2 THEN _expr_2
  WHEN _expr_1 = len + 3 THEN _expr_2
  ELSE _expr_2
END AS _expr_10,
