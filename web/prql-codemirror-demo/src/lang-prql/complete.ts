...
const dontComplete = ["Comment", "Docblock", "String", "FormatString", "RawString", "ServerString"]

const globals: readonly Completion[] = [
  "false", "null", "true"
].map(n => ({label: n, type: "constant"})).concat([
  "bool", "float", "int", "int8", "int16", "int32", "int64", "int128", "text",
  "date", "time", "timestamp"
].map(n => ({label: n, type: "type"}))).concat([
  // aggregate-functions
  "any", "average", "concat_array", "count", "every", "max", "min", "stddev",
  "sum",
  // file-reading-functions
  "read_csv", "read_parquet",
  // list-functions
  "all", "map", "zip", "_eq", "_is_null",
  // misc-functions
  "from_text",
  // string-functions
  "lower", "upper",
  ...
  "aggregate", "derive", "filter", "from", "group", "join", "select", "sort",
  "take", "window"
].map(n => ({label: n, type: "keyword"}))).concat([
  "std"
].map(n => ({label: n, type: "namespace"})))
