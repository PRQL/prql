...
function Sidebar({ library, onLoadFile }) {
  ...
  for (const [section, files] of Object.entries(library)) {
    ...
    for (const [index, filename] of Object.keys(files).entries()) {
      fileRows.push(
        <div key={index} className="fileRow" onClick={() => loadFile(section, filename)} >{filename}</div>
      );
    }
  ...
  return (
    <div className="sidebar">
      ...
      <section className="padded">
        ...
        <p>
          Try out PRQL in your browser, using wasm-compiled library <a target="_blank" rel="noopener noreferrer" href="https://npmjs.org/package/prql-js">prql-js</a>.
        </p>
