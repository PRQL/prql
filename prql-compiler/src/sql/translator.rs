    ...
    Ok(match item {
        ...
        ExprKind::Binary { op, left, right } => {
            if let Some(is_null) = try_into_is_null(&op, &left, &right, dialect)? {
                ...
                let left = translate_operand(left.kind, strength, !op.associates_left(), dialect)?;
                let right =
                    translate_operand(right.kind, strength, !op.associates_right(), dialect)?;
        ...
        ExprKind::Unary { op, expr } => {
            ...
            let expr = translate_operand(expr.kind, op.binding_strength(), false, dialect)?;
...
fn translate_operand(
    ...
    Ok(if needs_nesting {
        Box::new(sql_ast::Expr::Nested(translated))
    } else {
        translated
    })
}
