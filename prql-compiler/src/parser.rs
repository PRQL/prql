...
fn stmt_of_parse_pair(pair: Pair<Rule>) -> Result<Stmt> {
    ...
    let kind = match rule {
        ...
        Rule::query_def => {
            ...
            let version = params
                .remove("version")
                .map(|v| v.try_cast(|i| i.into_literal(), None, "literal"))
                .transpose()?
...
fn expr_of_parse_pair(pair: Pair<Rule>) -> Result<Expr> {
    ...
    let kind = match rule {
        ...
        Rule::expr_coalesce => {
            ...
            if pairs.clone().count() == 1 {
                ...
                ExprKind::FuncCall(FuncCall {
                    name: Box::new(
                        ExprKind::Ident(IdentWithNamespace {
                            namespace: None,
                            ident: ("coalesce".to_string()),
                        })
                        .into(),
                    ),
        ...
        Rule::operator_coalesce => ExprKind::Ident(IdentWithNamespace {
            namespace: None,
            ident: "-".to_string(),
        }),
