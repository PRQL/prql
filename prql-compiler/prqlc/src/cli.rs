...
impl Command {
    ...
    fn execute<'a>(&self, source: &'a FileTree, main_path: &'a str) -> Result<Vec<u8>> {
        ...
        Ok(match self {
            Command::Parse { format, .. } => {
                let ast = prql_to_pl_tree(source)?;
                match format {
            ...
            Command::Format(_) => {
                let (_, source) = source.files.clone().into_iter().exactly_one()?;
                let ast = prql_to_pl(&source)?;

                pl_to_prql(ast)?.as_bytes().to_vec()
            }
            Command::Debug(_) => {
                let (source_id, source) = source.files.clone().into_iter().exactly_one()?;
                let stmts = prql_to_pl(&source)?;

                let context = semantic::resolve_only(stmts, None)?;

                let source_id = source_id.to_str().unwrap().to_string();
                let references = label_references(&context, source_id, source.clone());
