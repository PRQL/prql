...
impl<'a> From<Rich<'a, crate::lexer::lr::Token, Span>> for Error {
    fn from(rich: Rich<'a, crate::lexer::lr::Token, Span>) -> Error {
        rich_error_to_error(
            *rich.span(),
            rich.reason(),
            |t: &crate::lexer::lr::Token| t.kind.to_string(),
            |t: &crate::lexer::lr::Token| matches!(t.kind, TokenKind::NewLine | TokenKind::Start),
        )
    }
}
...
impl<'a> From<Rich<'a, TokenKind, Span>> for Error {
    fn from(rich: Rich<'a, TokenKind, Span>) -> Error {
        rich_error_to_error(
            *rich.span(),
            rich.reason(),
            |t: &TokenKind| t.to_string(),
            |t: &TokenKind| matches!(t, TokenKind::NewLine | TokenKind::Start),
        )
    }
}
