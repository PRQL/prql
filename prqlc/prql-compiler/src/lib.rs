...
//!     from `build.rs`.
//!     See [this example project](https://github.com/PRQL/prql/tree/main/prqlc/prql-compiler/examples/compile-files).
//!
...
//! ## Large binary sizes
//!
//! For Linux users, the binary produced by this crate will probably be quite large (>20MB) by default.
//! That is because it includes a lot of debuginfo symbols from our parser. They
//! can be removed by adding the following to `Cargo.toml`, reducing the binary
//! size to around 7MB:
//! ```toml
